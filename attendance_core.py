...
def normalize_date(text: str, jalali: bool) -> str:
    """
    ورودی تاریخ را با قالب پیش‌فرض گرفته و تاریخ میلادی به صورت YYYY-MM-DD برمی‌گرداند.
    Gregorian input must be yyyy-mm-dd; Jalali input must be yyyy/mm/dd.
    """
    text = (text or "").strip()
    pattern = r"^\d{4}/\d{2}/\d{2}$" if jalali else r"^\d{4}-\d{2}-\d{2}$"
    if not re.fullmatch(pattern, text):
        if jalali:
            raise ValueError("قالب تاریخ شمسی باید yyyy/mm/dd باشد (مثال: 1405/04/01)")
        raise ValueError("قالب تاریخ میلادی باید yyyy-mm-dd باشد (مثال: 2026-06-22)")
...
def normalize_time(text: str) -> str:
    """زمان را فقط با قالب پیش‌فرض hh:mm:ss می‌پذیرد و استاندارد می‌کند."""
    text = (text or "").strip()
    if not text:
